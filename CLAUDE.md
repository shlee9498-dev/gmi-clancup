...
- **게임 규칙·참가 조건을 바꾸는 코드 변경은 같은 PR에 룰북·참가자 안내문 수정을 반드시 포함한다.**
  상한을 권장으로 바꾼 #132는 검증 로직만 고치고 안내문을 그대로 둬서, 신청자가 "초과 팀은
  접수되지 않습니다"를 읽으며 실제로는 통과하는 상태가 하루 지속됐다. 티어 판정 주체를 바꾼
  #136도 같은 문구를 남겨 "S 판정은 자동조회에서만"이 실제와 어긋났다.
  **판정식·검증 로직을 건드렸으면 그 규칙을 설명하는 화면을 같은 PR에서 찾아 고칠 것.**
